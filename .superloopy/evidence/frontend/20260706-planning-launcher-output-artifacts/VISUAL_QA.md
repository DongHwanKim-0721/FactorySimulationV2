...
Surface: native Tkinter planning workbook launcher output path suggestion and output artifact summary.
...
- `Suggest output paths` derives JSON and XLSX report paths from the selected input workbook stem.
- Existing user-entered output paths are left intact when suggestion runs.
- `Show outputs` reports generated JSON/XLSX artifact paths from the last successful run.
- No OS file-opening or folder reveal side effects were added.
- The deterministic workbook service remains the only calculation path.
...
- Focused planning launcher and workbook tests: 21 passed.
- Full regression suite: 156 passed.
