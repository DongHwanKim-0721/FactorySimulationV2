...
The current planning-run workflow can create a sample/template input workbook, read a real `.xlsx` workbook, run the deterministic planning core, and write either a report JSON snapshot or a planner-facing report `.xlsx` through CLI entry points. The first planning UI workbook-runner baseline is implemented as `planning_launcher.py` and can also be opened from the existing desktop app toolbar as a separate top-level window.
